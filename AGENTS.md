...
- OpenCode、MiniMax 等外部 Coding Agent 默认定位为受控实现者，不负责独立决定架构、数据兼容策略、安全边界或验收标准；主 Agent 负责冻结任务、保护工作区并独立验收。
- 用户未指定外部模型时，主 Agent 可以根据任务特征自行决定是否交给 OpenCode，并在本机 `opencode models` 当前可调用的模型中选择 DeepSeek 或 MiniMax，无需为低风险模型选择单独询问用户；用户明确指定模型时优先服从用户要求。
- 默认候选为 `deepseek/deepseek-v4-flash` 与 `minimax-cn-coding-plan/MiniMax-M3`：范围明确、行为冻结、1～3 个文件的实现或测试任务优先 DeepSeek；需要阅读较长上下文、遵循多文件既有模式或进行受控代码理解与改造时优先 MiniMax。模型选择只是执行策略，不得改变任务范围、架构决策或验收标准。
- 派发前必须用 `opencode models` 确认所选模型仍可调用；候选模型不可用时，主 Agent 只能选择同一 provider 下当前可用的同系列模型，并在验收记录中写明实际模型 ID，禁止静默改用未知模型。
