...
- [renderer/release] Visual 与 a11y Playwright 测试通过配置白名单启动器动态分配严格回环端口并正确传递失败状态，a11y 在正式审计前以独立 120 秒预算预热首次 Vite 页面且单个 WCAG 场景仍保持默认 30 秒，同时提高 Review diff 小字号文本及增删高亮背景的 WCAG AA 对比度，避免持久 Windows runner 残留进程、冷启动误报或假绿结果阻断 Beta 发布
