...
- [agent/desktop/renderer] 完整闭环修复 DeepSeek Harness v2 吸收缺陷：统一 `PiAgentRuntime` 全生命周期错误边界与 Scope 释放；修复工具审批预览透传冻结 `toolCatalog`；修复 `RequestSnapshotRecorder` 脱敏 payload 赋值；在 `RuntimeContribution` 中引入不可变 `RuntimeCompositionPlan` 并增强 `manifestHash` 计算；修复 `runtimeContextDigest` 消息顺序并增加标准摘要；实现洋葱模型 `RuntimeInterceptorChain` 与防重复 `next()` 检测；在 `inputs` 存储中补齐 `delivery_kind` 并正确管理未提交 claims；强化 `PluginService` mutation 操作的 `operationId` 幂等与冲突校验；修复 `SystemProfileLoader` 释放器反转顺序；修复 `PluginViewPane` 节点校验运算符优先级与动作后刷新。

### Changed
- [agent] 将 provider catalog 默认数据源切换到 models.dev，并保留 `@earendil-works/pi-ai` 内置目录作为降级回退，避免启动时缺少模型目录。
...
- [plugin-sdk] 新增唯一公共 workspace `packages/plugin-sdk`：Manifest v1、Application Plugin Protocol v1 wire（`cpx-plugin-rpc@1`，含 canonical JSON/digest 与 1 MiB 消息上限）、Service 声明与依赖（`publisher.service@major` + SemVer range）、权限声明、System/Profile 插件框架与 Manifest conformance 工具，并生成权威 JSON Schema 供跨语言 SDK 使用；插件工具禁止声明 `never-review`。
- [docs] 新增插件平台架构文档，明确固定元内核与可替换业务服务边界、两层 ABI、generation/lease 与可逆生命周期、System Profile 回退语义及信任模型（插件是完全可信本机代码，独立进程不是安全沙箱）；根、Agent、Packages `AGENTS.md` 明确插件平台唯一新 workspace 与"插件代码只能经 Plugin Host/System Profile loader 动态执行"的边界。
- [agent] 插件平台存储与安装基础设施：profile schema 3→4 新增 `plugin_packages`/`plugin_activations`/`plugin_grants`/`plugin_profile_generations`/`plugin_operations`/`plugin_kv` 六张独立表（事务迁移、高于 v4 的 schema 保持 user_version 与未知表），新增 `PluginRepository`（SQL 唯一来源）与 `.cpxplugin` 安装器：同卷 staging + 原子 rename、ZIP central directory 预扫描（原始 entry 名，拒绝路径穿越/绝对路径/ADS/Windows device name/symlink/重复路径/大小写折叠冲突）、加密与 ZIP64 拒绝、manifest 结构校验与逐文件 hash 校验、绝不执行安装脚本、安装后只能是 installed-disabled（digest 变化重置 enablement）、开发目录链接（canonical path + 目录 digest，文件变化只产生 staged digest 不自动执行）、卸载删除包文件但保留插件数据目录/KV/配置。
- [agent/agent-protocol] 插件管理 RPC 与 Developer Mode：新增 `plugin/list`、`plugin/installPackage`、`plugin/linkDirectory`、`plugin/unlinkDirectory`、`plugin/enable`、`plugin/disable`、`plugin/uninstall`、`plugin/config/get`、`plugin/config/update`、`plugin/grants/get`、`plugin/grants/update`、`plugin/profile/list`、`plugin/profile/stage`、`plugin/profile/applyOnRestart`、`plugin/operation/get` 十五个方法（capability `plugins.manage.v1`/`plugins.runtime.v1`/`plugins.views.v1`/`plugins.system-profiles.v1`，无 capability 客户端看不到管理方法）与 `plugin/inventory-changed`、`plugin/operation-updated`、`plugin/runtime-status-changed` live 事件；新增 `PluginService`：安装后只能是 installed-disabled、每个新 digest 需要独立信任确认（`__digest__` grant，新 digest 不继承旧信任）、global/workspace enablement（workspace override 高于 global）、Developer Mode 默认关闭时 process/System 包不可启用、mutation 按 operationId 幂等、错误信封安全归一化（不返回原始路径、命令或 stack）。
- [agent] Application 插件运行时（generation/lease/进程树）：新增 `ProcessSupervisor`（最小环境 allowlist 不含 Agent token/Provider key/SQLite 路径、随机 instance token 只用于 initialize 握手、初始化超时 10s、优雅 shutdown 超时 5s 超时后 taskkill 完整进程树、stdio 行协议单条 ≤1 MiB）；新增 Service dependency graph（required 缺失 → waiting、cycle 拒绝并返回循环路径、singleton 多 provider 冲突、拓扑序 provider 先于 consumer 激活，Provider 消失按反向拓扑停用、恢复后自动重激活）；新增 `PluginRuntimeManager`（main turn 与 subagent 同时 acquire generation lease、staging 全部健康后才替换 active 的 last-good 语义、retired generation 等 leases=0 后 quiesce/shutdown、crash loop 达 3 次自动 disable 并保留安全诊断、普通 disable 停止新 lease 等 drain、强制 disable 立即杀进程树）；`PiAgentRuntime` 预留的 `acquireLeases` 装配位接通插件 generation 租约。
- [agent/agent-protocol] 插件贡献适配（首批全栈）：`plugins@builtin` 运行时贡献把插件工具注册进快照冻结前的 tool builders（sdkName `plugin.<pluginId>.<tool>`，ToolOrigin 新增 plugin provenance，执行只转发 `plugin/toolExecute`，默认 policy 审批、未声明 capabilities 时 externalState 默认 true，继续走 ToolCatalog→ToolExposurePlan→ToolExecutor→PermissionDecisionEngine 全链）；Host broker 实现 `host/kvGet/kvPut/kvDelete`（按插件+scope 隔离、CAS）、`host/serviceCall`（按 service graph 转发 provider）、`host/credentialUse`（只返回 brokered reference，明文不跨 wire）；插件 MCP 声明转换为现有 MCP declaration 并合并进 `McpConfigService`（用户声明优先，OAuth/allow-deny/审批/lease 复用现有机制）；插件 Skill root 以包根为 containment 合并进 SkillService catalog（origin=plugin + pluginId provenance，资源不能越出包根）；`plugin/config/update` 按 manifest configSchema 校验；新增只读 `plugin/contribution/list`（capability `plugins.runtime.v1`）返回工具/技能/MCP/命令/设置/视图贡献目录；`InstalledSkill` 新增 `plugin` scope 与 pluginId 字段。
- [agent/agent-protocol/desktop] 插件命令与声明式视图：新增 `plugin/command/execute`（模板展开或转发 service action）、`plugin/view/render`（返回 Host-owned 白名单节点）、`plugin/view/action`；`plugin/list` 新增 `runtimeStatus` 与 `trustedDigest` 字段；Desktop 新增插件管理页（Developer Mode 开关、`.cpxplugin` 安装、开发目录链接、digest 信任确认与 H2 信任文案、global enable/disable、卸载保留数据）、声明式插件视图 tab（`plugin-view:<instanceId>` descriptor、一个 Host renderer 渲染全部插件视图、节点白名单校验拒绝未知节点、disable 时 tab 显示不可用并可重新加载）、slash 命令合并（builtin/skill/插件命令统一搜索、builtin 触发器优先）；Electron 仅新增 typed bridge：`pickPluginPackageFile`、`pickPluginDirectory`、`restartApp`（集中 IPC contract，不创建插件专用 preload/WebContents）。
- [agent] System Profile Composition Kernel：新增 `SystemProfileLoader`（Agent 进程内 System/Profile 插件激活内核）：generation 固定 plugin id/version/digest/provider selection/config；pending generation 在 restart 前不生效；bundle 必须位于包根内且 id 与包声明一致；全部 provider 激活成功才标记 last-good；boot 失败写安全诊断、清除 pending 并请求 sidecar 重启一次（退出码 3，Electron 断连自动重启），连续失败（≥1 次）回退默认 Profile 且不再请求重启（禁止无限 fallback loop）；未解析的 required contract fail-closed；System scope 生命周期为 process lifetime，shutdown 逆序释放；`plugin/profile/stage` 按 configSchema 校验（stage 不改变当前 runtime）；无 pending 或默认 Profile 时现有行为完全不变。
- [agent/plugin-sdk] System Service 契约框架与首个可替换业务服务：新增 `SystemServiceRegistry`（默认实现 = 现有代码路径，无激活 provider 时 `resolve` 返回 null 行为完全不变，provider 激活后覆盖默认、可按归属查询）；SDK 新增 `codepilotx.model-runtime@1`（模型目录 facade）与 `codepilotx.tool-runtime@1`（工具目录 facade）契约，System Profile 激活时按 `contributes.services` 声明注册 provider、声明了但未注册 fail-closed 不破坏 last-good；模型目录走既有 `model/list` 加载路径（激活 provider 时返回 provider catalog），自定义工具目录叠加进 ToolRegistry（激活后启动时注册）。
- [agent/plugin-sdk] System permission-policy 服务（可替换审批策略）：SDK 新增 `codepilotx.permission-policy@1` 契约（`decide` 返回 allow/review/deny）；`PermissionDecisionEngine` 新增可选 policyOverride 注入，provider 决策映射回既有固定 approval checkpoint shape（allow/review/deny + sandbox/risk/reviewer 字段保持），provider 抛错 fail-closed 转入 review；ApprovalService 与 ToolExecutor 经构造函数注入同一 override，bootstrap 接 SystemServiceRegistry 解析；无激活 provider 时默认审批策略逐位不变。
- [agent/plugin-sdk] System agent-loop 服务（整轮编排替换点）：SDK 新增 `codepilotx.agent-loop@1` 契约（`runTurn` 固定输入 thread/turn 身份、模型选择、权限上下文、恢复信息、中断状态；固定输出完成/暂停/错误/中断分类、usage 与 Host 待发布事件）；`PiOrchestratorAdapter` 新增 loopOverride 委托路径：激活 provider 时整轮 turn 委托 `runTurn`（Pi 编排完全不启动），输出映射回既有 `AgentRuntimeResult`（paused/error/interrupted 语义保持，错误分类为 AgentError 且不泄露原始异常，事件由 Host 发布、usage 走既有 onUsage 回调），无 provider 时 Pi 编排原路径逐位不变。
- [agent/plugin-sdk] System session-persistence 服务（会话持久化替换点）：SDK 新增 `codepilotx.session-persistence@1` 契约（open/append/transaction/flush/replay/cursor/checkpoint/recovery/shutdown 固定操作）与 `runSessionPersistenceConformance` conformance 套件（往返一致、游标单调、事务原子、重启后可重放、checkpoint 跨重启保留、Provider 必须把持久化产物写入自己的 namespaced 数据根）；SystemProfileLoader 为每个 System 插件创建 `systemDataRoot/<pluginId>/<generationId>/` 专属数据根并注入 `SystemPluginContext.dataRoot`，声明 session-persistence 的 Provider 在 staging 前必须通过 conformance（失败 fail-closed 不破坏 last-good）；默认实现为现有 SQLite/AgentDatabase 原路径，无激活 provider 时行为完全不变。
...
- [plugin-sdk] 新增 Application Runner conformance 工具（`runRunnerConformance`）：spawn 插件进程并按 `cpx-plugin-rpc@1` wire 验证 initialize 握手响应头完整（protocol/version/pluginId/generation/requestId 回显与 capabilities）、未知方法只返回 code/message/retryable 安全错误 envelope（携带 stack 等额外字段视为失败）、toolExecute/quiesce/shutdown 响应与 shutdown 后进程退出，进程启动判定不依赖 spawn 事件（bun 上延迟触发）；配套自包含 fixture（normal/slow-init/missing-header/leak-stack 模式）与 5 个测试用例，供跨语言 runner 对照验证。
- [plugin-sdk] 新增 `.cpxplugin` 打包工具与插件开发 CLI：`packPluginDirectory` 按实际文件自动补全 `manifest.files` sha256 清单并生成与宿主安装校验一致的 ZIP（store、UTF-8 文件名、拒绝符号链接、5000 文件/256 MiB 上限、manifest.json 自指占位），输出 digest 与宿主安装后完全一致（agent 集成测试验证可安装且 digest 相同）；`bun run plugin` CLI 提供 `validate`/`pack`/`conformance:runner`/`conformance:persistence` 四个命令，输出只含安全错误码与相对路径、失败退出码 1；修复 runner-conformance 中 shutdown 检查未清理的超时 timer（此前会让进程多等 5 秒）。
- [docs] 插件平台架构文档补齐 Package 格式（`.cpxplugin`）、开发工具与本地开发目录链接、Developer Mode 与凭据 broker、System Plugin API 契约表、兼容矩阵与 Release 状态五节。

### Changed

- [agent] Runtime Kernel 调整为可逆装配顺序：每次运行先创建 Runtime Scope，再获取资源租约（MCP/插件 generation lease 装配位），随后只对实际启用的运行时贡献执行注册（新增 `resolveEnabled()` 与 tool/prompt/guard/observer builder handles），组合 prompt/tools/guards/observers 之后才冻结 RuntimeSnapshot 并最后创建 AgentHarness；disabled 贡献不再出现在 prompt、工具目录与快照中，快照新增 plugin generation/digest 与服务绑定哈希标识，默认无插件行为与原有快照一致。
...
- [Agent] 统一工具执行管线：`ToolExecutor` 作为唯一入口实现 resolve → inspect → authorize → execute → finalize 五阶段，注册工具与 Bash/PowerShell 共用 pre-tool Hook 决策单调合并（deny > require-approval > continue）、SecretScrubber 审计与 post Hook 终结流程，`authorizationOnly` 仍完整解析检查但绝不进入执行，错误码、审批、Hook、Shell 风险与文件 mutation 语义保持不变。
- [Agent] 新增内部 Runtime Kernel：每次运行创建可回收的 Runtime Scope（identity 绑定 thread/turn/agent/session，dispose 先中止再收敛已登记操作、按逆序释放 disposer 且幂等聚合失败），PiAgentRuntime 的 Harness 订阅、Provider/tool hooks、abort listener 与静态贡献全部注册到 scope；新增 `core/skills/mcp/subagents@builtin` 静态贡献注册表与按 taskMode/profile 计算的 Runtime Preset，每次 run 开始时冻结含 prompt/tool catalog 哈希的 RuntimeSnapshot，运行期间配置变化只影响下一次运行。
- [Agent] 新增只读 RPC `runtime/contribution/list`（capability `runtime.contributions.v1`），桌面插件目录可展示 Agent 内置贡献及其实际启用状态；不提供安装或任意启停接口，条件贡献沿用 Skills/MCP/Subagent 各自现有配置。
- [Agent] 语义运行时事件与单一投影器：`PiRuntimeEventSink` 的大量可选回调替换为判别联合 `PiRuntimeEvent`，`PiEventAdapter` 只负责 Harness 事件到语义事件的映射、计划解析与稳定 item ID；新建 `PiRuntimeProjector` 统一承接 pending item、live delta、durable event 与 savepoint 事务（flush → canonical item → guide mailbox → durable events/outbox，事务提交后才发布，失败调用 `discardPending()` 且不发布已回滚事件），`PiOrchestratorAdapter` 只保留编排、审批、恢复与 runtime 创建；`thread-rpc-v4` 事件名称与 `session-view` 投影不变。
- [Agent] 完整 Provider 请求快照：history schema 32 新增 `model_request_snapshots` 独立表（含 `captured`/`missing` 状态约束、ordinal 唯一、thread/turn 级联删除），`before_provider_payload` 在 SecretScrubber 脱敏后、发送前按固定顺序落快照（含运行时 preset/贡献/哈希清单），捕获失败只写无 payload 的 missing 记录或安全诊断且不阻断 Provider 请求；新增用户层 `diagnostics.model_request_snapshots.enabled`（默认关闭、仅严格布尔 true 启用、`diagnostics` 进入项目禁用根），关闭只停止采集不删除历史，归档保留、永久删除级联清理；新增只读 RPC `runtime/request-snapshot/list` 与 `runtime/request-snapshot/read`（capability `runtime.request-snapshots.v1`）及 durable event `runtime/request-snapshot/created`，事件只带摘要、read 是完整 payload 唯一出口并按 thread 隔离。
- [desktop/renderer] 新增右侧“请求”检查器：设置页新增“隐私与诊断”开关（首次开启需确认脱敏明文、归档保留与明文导出风险，关闭不弹确认也不删除历史）；仅当记录开启、协商 `runtime.request-snapshots.v1` 且存在真实任务时 launcher 才显示，关闭记录立即隐藏入口并关闭已打开 Tab，`conversationUiState` 在未启用时丢弃恢复出的 descriptor 且 handoff 不复制；面板为请求列表 + 详情布局（分页、增量 created event 插入顶部、重连以 list 对账、captured 展示 payload/哈希/运行时清单、missing 只显示安全错误码、复制/导出复用完整保存字符串），独立 lazy chunk 不进入 `/new` 首屏；插件目录页合并展示 Agent 内置贡献（只读、标记“内置/Agent Runtime”与提供的类型）。

- [Agent] Harness 骨架 P1 正确性修复：每轮构建 per-turn frozen ToolCatalog（base + 贡献合并、重复 SDK name 在 Harness 创建前 fail-closed、exposure/adapter/执行/快照共用同一目录，contributed tool 不再 TOOL_NOT_FOUND）；harness subscriber 等待 projector 事务且 observer 失败只记录诊断；failed/aborted runtime 从 active map 移除并释放，初始化任意位置失败 scope/listener 归零；请求快照 publish 失败只写安全诊断并依赖 outbox replay，不再产生 captured+missing 双记录；持久化 Runtime Manifest v2（组合层/插件 generation 绑定/服务绑定拓扑，reader 兼容 v1）。
- [Agent] 新增 DeepSeek Harness 语义内核：history schema 33 新增 `runtime_steps`（持久 step 状态机）与 `runtime_context_snapshots`（model-visible invariant），`inputs` 增加 nullable `delivery_kind`/`claimed_step_id` 兼容列；统一 Turn Inbox 在 step 边界按 created_at,id 原子认领，savepoint 后 claim 落定、abort/crash 未提交 claim 释放回队列；每次 Provider 请求对应一个 step ordinal，retry 同 step、reactive compaction 产生新 context attempt；发送前持久化规范化 semantic context（scrub 后写库、失败阻止请求、digest 不一致 fail-closed）；启动恢复把 running step 标记 interrupted；新增可选 capability `runtime.step-lifecycle.v1` 与 step-started/completed/failed/interrupted durable 事件；新增 waterfall interceptor 扩展点（pre-step/provider-request/tool-pre-execute 等，顺序执行、next() 最多一次、short-circuit 语义）。
- [desktop/renderer] 打通右侧“请求”Tab：created event 改为经 `envelope.payload.snapshot` 判别收窄（移除 params cast）；统一三态 availability（loading/enabled/disabled+确定性 reason），gate 解析前不误丢弃恢复的 descriptor、enabled 后恢复、关闭或 capability 消失立即关闭 Tab；插件列表 Toggle 修复方向（开启调 enablePlugin、关闭调 disablePlugin）。
