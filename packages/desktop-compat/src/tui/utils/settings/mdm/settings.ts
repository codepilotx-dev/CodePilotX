/**
 * MDM (Mobile Device Management) profile enforcement for CodePilotX managed settings.
 *
 * Reads enterprise settings from OS-level MDM configuration:
 * - macOS: `local.codepilotx` preference domain, with legacy
 *   `com.anthropic.claudecode` fallback
 *   (MDM profiles at /Library/Managed Preferences/ only — not user-writable ~/Library/Preferences/)
 * - Windows: `HKLM\SOFTWARE\Policies\CodePilotX` (admin-only)
 *   and `HKCU\SOFTWARE\Policies\CodePilotX` (user-writable, lowest priority)
