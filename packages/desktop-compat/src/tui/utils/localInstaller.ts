...
export function getLocalClaudePath(): string {
  return join(getLocalInstallDir(), 'codepilotx')
}
...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return (
    execPath.includes('/.codepilotx/local/node_modules/') ||
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'codepilotx-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const wrapperPath = join(localInstallDir, 'codepilotx')
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/codepilotx" "$@"`,
      0o755,
    )
    if (created) {
...
/**
 * Install or update CodePilotX CLI package in the local directory
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install CodePilotX CLI package: ${result.stderr}`,
      )
      logError(error)
...
export async function localInstallationExists(): Promise<boolean> {
  try {
    await access(join(getLocalInstallDir(), 'node_modules', '.bin', 'codepilotx'))
    return true
  } catch {
    try {
