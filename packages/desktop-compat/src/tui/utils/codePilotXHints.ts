/**
 * CodePilotX hints protocol.
 *
 * CLIs and SDKs running under CodePilotX can emit a self-closing
 ...
 */
export function extractCodePilotXHints(
  output: string,
  command: string,
): { hints: CodePilotXHint[]; stripped: string } {
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[codePilotXHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[codePilotXHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[codePilotXHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as CodePilotXHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: CodePilotXHint | null = null
...
export function setPendingHint(hint: CodePilotXHint): void {
...
export function getPendingHintSnapshot(): CodePilotXHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetCodePilotXHintStore(): void {
  pendingHint = null
