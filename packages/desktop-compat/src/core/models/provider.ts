...
export function providerToCodexToml(
  provider: ModelProviderSummary,
  ...
  if (modelID) {
    edits.push({ keyPath: 'model', value: modelID, mergeStrategy: 'replace' })
  }
  if (providerID) {
    edits.push({
  ...
  const envKey = provider.envVars?.find(value => value.trim())?.trim()
