...
/**
 * Static tool scheduling strategy or a resolver evaluated for each validated
 * tool input. A batch is sequential when any resolver returns "sequential".
 */
export type ToolExecutionMode = ToolExecutionStrategy | ((input: unknown) => ToolExecutionStrategy | Promise<ToolExecutionStrategy>);
/**
...
export interface AgentToolResult<T> {
    ...
    details?: T;
    /** Structured result compatible with MCP-style tool responses. */
    structuredContent?: unknown;
    /** Optional progress metadata. Most commonly used in partial update callbacks. */
    progress?: AgentToolProgress;
    /** Usage from the final tool execution itself, if available. Not used for main LLM context accounting. */
    usage?: Usage;
