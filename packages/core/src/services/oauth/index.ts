...
/**
 * Callback type for opening a URL in the browser.
 * TUI provides the actual implementation (openBrowser from utils/browser.ts);
 * Desktop provides its own (Electron shell.openExternal).
 */
export type OpenBrowserFn = (url: string) => Promise<boolean>
...
export class OAuthService {
  ...
  async startOAuthFlow(
    ...
    options?: {
      ...
      /**
       * Browser opener function. If not provided, no browser is opened
       * (caller must handle URL display via authURLHandler).
       */
      ...
      async () => {
        if (options?.skipBrowserOpen) {
          ...
          if (options?.openBrowser) {
            await options.openBrowser(automaticFlowUrl) // Try automatic flow
          }
        }
      },
    )
