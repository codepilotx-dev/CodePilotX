...
test('default registry keeps app-server API present with explicit migration error', async () => {
  const server = new JsonRpcAppServer()

  await expect(
    server.startThread({ threadId: 'thread-jsonrpc', settings: {} }),
  ).rejects.toMatchObject({
    code: -32000,
    message: expect.stringContaining('当前 core 暂不支持/迁移中'),
    data: {
      cause: expect.stringContaining('appServer registry'),
    },
  })
})
