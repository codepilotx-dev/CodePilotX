...
describe('applyRightDockAction', () => {
  ...
  test('openTool allows the performance diagnostics tool in debug mode', () => {
    expect(
      applyRightDockAction(
        empty,
        { type: 'openTool', tool: 'performanceDiagnostics' },
        { debugMode: true },
      ),
    ).toEqual({
      open: true,
      activeTool: 'performanceDiagnostics',
      openTools: ['performanceDiagnostics'],
    })
  })
