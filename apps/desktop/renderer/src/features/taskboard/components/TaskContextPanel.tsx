...
function TaskContextEditForm({
  editing,
  onCancel,
  onChange,
  onSave,
}: {
  editing: ContextEditingState
  onCancel: () => void
  onChange: (editing: ContextEditingState) => void
  onSave: () => void
}) {
  return (
    <form onSubmit={event => { event.preventDefault(); onSave() }}>
      <Input aria-label="上下文标题" maxLength={120} value={editing.title} onChange={event => onChange({ ...editing, title: event.currentTarget.value })} />
      <Textarea aria-label="上下文内容" maxLength={2_000} rows={4} value={editing.content} onChange={event => onChange({ ...editing, content: event.currentTarget.value })} />
      <Button color="ghostSecondary" size="compact" type="button" onClick={onCancel}>取消</Button>
      <Button color="secondary" disabled={!editing.title.trim() || !editing.content.trim()} size="compact" type="submit">保存</Button>
    </form>
  )
}
...
export function TaskContextPanel({ taskId, readOnly }: { taskId: string; readOnly: boolean }) {
  ...
  const [editing, setEditing] = useState<ContextEditingState | null>(null)
