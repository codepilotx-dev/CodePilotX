...
export function PetSettings({
  ...
  const refresh = async (): Promise<void> => {
    ...
    try {
      ...
      if (
        ...
        updatePet({ selectedPetId: next[0]?.id ?? null })
      }
    } catch (error) {
  ...
  const install = async (): Promise<void> => {
    ...
    try {
      ...
      setPreview(null)
      setSourceUrl('')
      updatePet({ selectedPetId: installed.id })
            ...
            control={
              <div className="pet-settings-inline">
                <select
                  ...
                  onChange={event =>
                    updatePet({ selectedPetId: event.target.value || null })
                  }
                >
          ...
          <SettingsRow
            ...
            control={
              <input
                ...
                value={settings.size}
                onChange={event =>
                  updatePet({ size: Number(event.target.value) })
                }
              />
            }
          />
