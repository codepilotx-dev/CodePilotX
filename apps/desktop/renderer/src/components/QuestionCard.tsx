...
import type { TaskQuestion } from '../domain/task-flow'
...
export function QuestionCard({ question, onSubmit }: QuestionCardProps) {
  const recommendedID = useMemo(() => question.options.find((option) => option.recommended)?.id ?? question.options[0]?.id ?? '', [question.options])
  ...
  return (
    <section className={`question-card question-${question.status}`} aria-label="需要用户回答的问题">
      ...
      <div className="question-options" role="radiogroup" aria-label={question.prompt}>
        ...
        {answered && question.answer && !question.options.some((option) => option.label === question.answer) ? <span className="question-custom-answer">{question.answer}</span> : null}
      </div>
