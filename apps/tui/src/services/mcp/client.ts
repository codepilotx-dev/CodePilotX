...
/**
 * Claude.ai proxy MCP required Claude.ai OAuth tokens, which are removed from
 * this build. Keep the export so local tests/imports compile, but never attach
 * credentials or attempt refresh.
 */
  ...
  async (
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        throw new Error('claude.ai proxy MCP requires Claude OAuth and is removed')
      } else if (
        ...
        const {
          createChromeContext,
          createClaudeInChromeMcpServerForCli,
        } = await import(
        ...
        inProcessServer = await createClaudeInChromeMcpServerForCli(context)
