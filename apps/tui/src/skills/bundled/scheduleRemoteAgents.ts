...
export function registerScheduleRemoteAgentsSkill(): void {
  registerBundledSkill({
    ...
    whenToUse:
      'When the user wants to schedule a recurring remote agent, set up automated tasks, create a cron job for Oh-My-AgentCode, or manage their scheduled agents/triggers.',
    userInvocable: true,
    isEnabled: () =>
    ...
    async getPromptForCommand(args: string, context: ToolUseContext) {
      if (!getClaudeAIOAuthTokens()?.accessToken) {
        return [
          {
            type: 'text',
            text: 'You need to authenticate with a claude.ai account first. API accounts are not supported. Configure ANTHROPIC_API_KEY',
          },
        ]
      }
