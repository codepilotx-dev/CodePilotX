...
export function buildMemoryLines(
  ...
  const lines: string[] = [
    `# ${displayName}`,
    '',
    `You have a persistent, file-based memory system accessible via the \`memory\` tool at the virtual path \`/memories\`. Use \`memory.view /memories/MEMORY.md\` to read the index.`,
    '',
...
export function buildSearchingPastContextSection(autoMemDir: string): string[] {
  ...
  return [
    ...
    '1. Use `memory.view /memories` to list all memory files, then view specific files with `memory.view /memories/<topic>.md`.',
    '2. Search topic files in your memory directory:',
    '```',
    memSearch,
    '```',
