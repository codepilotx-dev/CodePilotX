...
import type { ModelUsage } from '@codepilotx/tui/entrypoints/agentSdkTypes.js'
...
/**
 * Convert a PersistedStatsCache to CodePilotXStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all CodePilotX sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateCodePilotXStats(): Promise<CodePilotXStats> {
 ...
 */
export async function aggregateCodePilotXStatsForRange(
  range: StatsDateRange,
): Promise<CodePilotXStats> {
  if (range === 'all') {
    return aggregateCodePilotXStats()
  }
  ...
  return processedStatsToCodePilotXStats(stats)
}

/**
 * Convert ProcessedStats to CodePilotXStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToCodePilotXStats(
...
function getEmptyStats(): CodePilotXStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
