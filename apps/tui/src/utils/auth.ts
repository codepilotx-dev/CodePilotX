...
export function isClaudeAISubscriber(): boolean {
  // Removed Claude.ai OAuth — API key mode only
  return false
}

/**
 * Check if the current OAuth token has the user:profile scope.
 * Always returns false in API-key-only mode.
 */
...
export function is1PApiCustomer(): boolean {
  // API key users are direct API customers
  return true
}

/**
 * Gets OAuth account information.
 * Returns undefined in API-key-only mode.
 */
...
// Returns whether the user has Opus access at all.
export function hasOpusAccess(): boolean {
  // API key users always have access to available models
  return true
}

export function getSubscriptionType(): SubscriptionType | null {
  // API-key-only mode - no subscription
  return null
}
...
/** Check if using third-party services - always false (removed) */
 ...
 *
 * In API-key-only mode, always returns valid (no OAuth to validate).
 */
