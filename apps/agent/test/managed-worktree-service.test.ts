...
describe("ManagedWorktreeService", () => {
  ...
  test("history 24 逐代迁移到当前版本并只增加可忽略的独立表", () => {
    ...
    const newObjects = /(?:managed_worktrees|thread_execution_bindings|worktree_operations|thread_handoff_operations|thread_forks|thread_message_fork_operations|thread_message_forks|turn_pi_boundaries|resume_checkpoint_leases|taskboard_|task_context_)/
