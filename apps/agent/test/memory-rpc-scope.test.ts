...
describe("Memory RPC 项目作用域", () => {
  ...
  test("新 RPC 方法已在共享 schema 登记且 read/delete 校验 scope", async () => {
    ...
    const entry = memory.remember({ scope: "project", projectKey: projectMemoryKey(projectA.id), content: "仅属于 A" })!
    ...
    expect(memory.read({ id: entry.id, scope: "project", projectKey: projectMemoryKey(projectA.id) })).not.toBeNull()
  ...
  test("project list/reset 缺少 projectId/threadId 时拒绝且不能跨项目清空", async () => {
    ...
    memory.remember({ scope: "project", projectKey: projectMemoryKey(projectA.id), content: "A" })
    memory.remember({ scope: "project", projectKey: projectMemoryKey(projectB.id), content: "B" })
    ...
    expect(memory.list({ scope: "project", projectKey: projectMemoryKey(projectA.id) })).toHaveLength(1)
    expect(memory.list({ scope: "project", projectKey: projectMemoryKey(projectB.id) })).toHaveLength(0)
