...
export const parseSkillDocument = (content: string) => {
  ...
  if (!closing || closing.index < 0) throw new Error("SKILL.md frontmatter 缺少结束分隔符")
  const parsed = parseYaml(rest.slice(0, closing.index), { maxAliasCount: 0 })
  if (parsed !== null && (typeof parsed !== "object" || Array.isArray(parsed))) throw new Error("SKILL.md frontmatter 必须是 YAML mapping")
...
const parseAllowedTools = (metadata: Record<string, unknown>) => {
  ...
  const tools = Array.isArray(value)
    ...
    : typeof value === "string"
      ? value.split(",")
      : []
  const normalized = [...new Set(tools.map((tool) => typeof tool === "string" ? tool.trim() : "").filter(Boolean))]
...
export class SkillService {
  ...
  async scan(workspaceRoot: string, userRoot: string, options: { includeWorkspace?: boolean } = {}): Promise<SkillCatalog> {
    ...
    for (const base of bases) {
      for (const compatibilityDir of COMPATIBILITY_DIRS) {
        ...
        try { canonicalSkillsRoot = await realpath(skillsRoot) } catch (cause) { if (missing(cause)) continue; throw cause }
        if (!contained(base.root, canonicalSkillsRoot)) throw new Error(`Skills 根目录逃出配置根: ${skillsRoot}`)
        const entries = (await readdir(canonicalSkillsRoot, { withFileTypes: true })).sort((a, b) => a.name.localeCompare(b.name))
        for (const entry of entries) {
          if (!entry.isDirectory() && !entry.isSymbolicLink()) continue
          const directory = await realpath(join(canonicalSkillsRoot, entry.name))
          if (!contained(canonicalSkillsRoot, directory)) throw new Error(`Skill 目录逃出 Skills 根: ${entry.name}`)
          const documentPath = join(directory, "SKILL.md")
          let canonicalDocument: string
          try { canonicalDocument = await realpath(documentPath) } catch (cause) { if (missing(cause)) continue; throw cause }
          if (!contained(directory, canonicalDocument)) throw new Error(`SKILL.md 逃出 Skill 根: ${documentPath}`)
          const bytes = await readFile(canonicalDocument)
          if (bytes.byteLength > MAX_SKILL_BYTES) throw new Error(`SKILL.md 超过 1 MiB: ${canonicalDocument}`)
          const content = decoder.decode(bytes)
          const parsed = parseSkillDocument(content)
          const declaredName = parsed.metadata.name
          const name = typeof declaredName === "string" && declaredName ? declaredName : entry.name
          if (!skillNamePattern.test(name)) throw new Error(`无效 Skill 名称: ${name}`)
          const declaredDescription = parsed.metadata.description
          const description = typeof declaredDescription === "string" ? declaredDescription : ""
          const allowedTools = parseAllowedTools(parsed.metadata)
          const metadata: SkillMetadata = {
            name, description, path: canonicalDocument, root: directory, origin: base.origin,
            format: compatibilityDir.slice(1) as SkillMetadata["format"], hash: sha256(bytes), metadata: parsed.metadata,
            ...(allowedTools ? { allowedTools } : {}),
          }
          const current = found.get(name)
          if (current) shadowed.push({ name, selectedPath: current.path, ignoredPath: metadata.path })
  ...
  resolveInvocation(value: string): SkillMetadata | null {
    const match = /^\s*[$/]([A-Za-z0-9][A-Za-z0-9_-]{0,63})(?=\s|$)/.exec(value)
  ...
  async read(name: string): Promise<LoadedSkill> {
    ...
    if (bytes.byteLength > MAX_SKILL_BYTES) throw new Error(`SKILL.md 超过 1 MiB: ${metadata.path}`)
    const content = decoder.decode(bytes)
    const parsed = parseSkillDocument(content)
  ...
  async resolveResource(name: string, resourcePath: string): Promise<string> {
    ...
    if (!stats.isFile() && !stats.isDirectory() && !stats.isSymbolicLink()) throw new Error("Skill 资源类型不受支持")
    const canonical = await realpath(lexical)
    if (!contained(metadata.root, canonical)) throw new Error("Skill 资源路径通过链接逃出 Skill 根")
    return canonical
  }
}
