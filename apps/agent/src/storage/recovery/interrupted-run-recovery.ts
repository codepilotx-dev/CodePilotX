...
export const recoverInterruptedRuns = (database: AgentDatabase) => {
  ...
  database.sqlite.transaction(() => {
    ...
    database.sqlite.query(`UPDATE tool_calls SET status = 'interrupted', finished_at = ?, error = COALESCE(error, 'Agent 重启时工具执行被中断') WHERE status = 'running' AND turn_id IN (SELECT id FROM turns WHERE status = 'interrupted')`).run(timestamp)
