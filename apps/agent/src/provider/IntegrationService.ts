...
} from "@codepilotx/provider-runtime";
...
type Runtime = Pick<AgentModelCatalog, "list">;
...
type AttemptRecord = {
  ...
  readonly callback: (
    code?: string,
  ) => Effect.Effect<Credential.Value, unknown>;
...
const generatedEnvNames = (draft: IntegrationDraft) => {
  const ids = [
    ...new Set([String(draft.id), ...draft.providerIDs.map(String)]),
  ];
...
export class IntegrationService {
  ...
  constructor(
    ...
    this.nextAttemptID =
      options.attemptID ?? (() => `attempt_${crypto.randomUUID()}`);
  }
  ...
  async list(): Promise<readonly Integration.Info[]> {
    try {
      ...
      for (const provider of providers) {
        const integrationID =
          provider.integrationID ?? Integration.ID.make(String(provider.id));
      ...
      for (const integration of registered) {
        ...
        for (const method of integration.methods)
          mergeMethod(draft.methods, method);
      ...
      for (const draft of drafts.values()) {
        ...
        if (!draft.methods.some((method) => method.type === "key"))
          mergeMethod(draft.methods, { type: "key" });
        if (!draft.methods.some((method) => method.type === "env")) {
          mergeMethod(draft.methods, {
            type: "env",
            names: generatedEnvNames(draft),
          });
        }
      }
      ...
      return [...drafts.values()].map((draft) => ({
        ...
        connections: [
          ...
          ...draft.methods
            .filter(
              (method): method is Integration.EnvMethod =>
                method.type === "env",
            )
            .flatMap((method) => method.names)
            .filter(
              (name, index, names) =>
                Boolean(this.env[name]) && names.indexOf(name) === index,
            )
  ...
  async connect(input: IntegrationConnectInput): Promise<Connection.Info> {
    try {
      ...
      if (!key)
        throw new AgentError("INVALID_CREDENTIAL", "API Key 不能为空", 400);
      const integration = await this.integration(input.integrationID);
      if (!integration.methods.some((method) => method.type === "key")) {
        throw new AgentError(
          "INTEGRATION_KEY_UNSUPPORTED",
          `集成 ${input.integrationID} 不支持 API Key`,
          400,
        );
      }
  ...
  async disconnect(input: IntegrationDisconnectInput): Promise<void> {
    try {
      ...
      const summary = this.credentials
        .list()
        .find(
          (item) =>
            item.integrationID === String(integrationID) &&
      ...
      const removed = await Effect.runPromise(
        this.credentials.remove(String(integrationID)),
      );
      if (
        !removed ||
        this.credentials
          .list()
          .some((item) => item.integrationID === String(integrationID))
      ) {
        throw new AgentError(
  ...
  async authorize(
    ...
    try {
      ...
      const registrations = await Effect.runPromise(
        this.pluginHost.authRegistrations(integrationID),
      );
      ...
      const authorization = await Effect.runPromise(
        registration.authorize(input.inputs),
      );
      ...
      if (attempt.mode === "auto") {
        queueMicrotask(() => {
          void this.runAttempt(record).catch(() => undefined);
        });
      }
      return attempt;
    } catch (cause) {
      throw toAgentError(
        cause,
        "INTEGRATION_OAUTH_AUTHORIZE_FAILED",
        "无法启动 OAuth 授权",
        502,
      );
    }
  ...
  async complete(input: IntegrationCompleteInput): Promise<Connection.Info> {
    ...
    if (!code)
      throw new AgentError("INVALID_OAUTH_CODE", "OAuth 验证码不能为空", 400);
  ...
  private async integration(integrationID: string): Promise<Integration.Info> {
    const integration = (await this.list()).find(
      (item) => item.id === integrationID,
    );
    if (!integration)
      throw new AgentError(
        "INTEGRATION_NOT_FOUND",
        `未找到认证集成 ${integrationID}`,
        404,
      );
    return integration;
  }
  ...
  private async validateKeyRegion(integrationID: string, key: string) {
    ...
    try {
      ...
      if (alternate.status === 401) {
        throw new AgentError(
          "INVALID_CREDENTIAL",
          "MiniMax 拒绝了该 API Key，请确认密钥正确且套餐仍有效",
          400,
        );
      }
    } catch (cause) {
  ...
  private expire(record: AttemptRecord) {
    if (
      record.status.status === "pending" &&
      this.now() >= record.attempt.time.expires
    ) {
  ...
  private pruneAttempts() {
    ...
    for (const [attemptID, record] of this.attempts) {
      this.expire(record);
      if (now >= record.attempt.time.expires + this.attemptTtlMs)
  ...
  private runAttempt(
    ...
    code?: string,
  ): Promise<Connection.Info> {
    if (record.running) return record.running;
    record.running = (async () => {
      try {
        this.expire(record);
        if (record.status.status !== "pending") {
          if (record.status.status === "complete" && record.connection)
        ...
        const registrations = await Effect.runPromise(
          this.pluginHost.authRegistrations(record.integrationID),
        );
        const registration = registrations.find(
          (item) =>
        ...
        const generatedLabel =
          value.type === "oauth" && registration && "label" in registration
  ...
  private async credentialForProvider(
    providerID: Provider.ID,
  ): Promise<Credential.Value | undefined> {
    try {
      const integrationID =
        this.integrationByProvider.get(String(providerID)) ??
        Integration.ID.make(String(providerID));
      const stored = await Effect.runPromise(
        this.credentials.get<Credential.Value>(String(integrationID)),
      );
      if (!stored && String(integrationID) !== String(providerID)) {
        const fallback = await Effect.runPromise(
          this.credentials.get<Credential.Value>(String(providerID)),
        );
        if (!fallback) return undefined;
        if (!Schema.is(Credential.Value)(fallback.value))
          throw new AgentError(
            "INVALID_CREDENTIAL",
            "存储的 Provider 凭据无效",
            500,
        ...
        return this.refreshOAuth(
          Integration.ID.make(fallback.integrationID),
          fallback.value,
          fallback.label,
        );
      }
      if (!stored) return undefined;
      if (!Schema.is(Credential.Value)(stored.value))
        throw new AgentError(
          "INVALID_CREDENTIAL",
          "存储的 Provider 凭据无效",
          500,
      ...
      return this.refreshOAuth(
        Integration.ID.make(stored.integrationID),
        stored.value,
        stored.label,
      );
    } catch (cause) {
  ...
  private async refreshOAuth(
    ...
    if (!registration?.refresh)
      throw new AgentError(
        ...
        `集成 ${integrationID} 的 OAuth 凭据已过期`,
        401,
      );
    const refreshed = await Effect.runPromise(registration.refresh(value));
    if (!Schema.is(Credential.OAuth)(refreshed))
      throw new AgentError(
        "INVALID_CREDENTIAL",
        "OAuth 刷新返回了无效凭据",
        502,
