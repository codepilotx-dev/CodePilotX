...
import type { CredentialCandidate, ProviderRuntime } from "@codepilotx/provider-runtime"
...
export class ApiKeyService {
  ...
  async test(credentialID: string): Promise<PublicApiKeySummary> {
    ...
    try {
      const language = await this.providers.getLanguageForCredential({ providerID, id: model.id }, candidate)
      await generateText({
        model: language as LanguageModel,
        prompt: "Reply OK.",
        maxOutputTokens: 8,
