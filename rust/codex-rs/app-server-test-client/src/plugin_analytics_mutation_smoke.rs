...
pub(super) fn run(
    ...
    let mut client = spawn_client(codepilotx_bin, config_overrides, &capture_path)?;
...
pub(super) fn run_cleanup(
    ...
    let mut client = CodexClient::spawn_stdio(codepilotx_bin, &overrides)?;
    client.initialize()?;
    ...
    match restore_uninstalled_state(&mut client, remote_plugin_id) {
        ...
        RestorationStatus::Dirty(err) => {
            print_dirty_recovery(codepilotx_bin, config_overrides, remote_plugin_id, &err);
            Err(err)
        }
...
fn spawn_client(
    ...
    CodexClient::spawn_stdio_with_env(codepilotx_bin, &overrides, &environment)
}
