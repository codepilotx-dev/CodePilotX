...
use codepilotx_extension_api::ThreadResumeInput;
use codepilotx_extension_api::ThreadStartInput;
use codepilotx_extension_api::ThreadStopInput;
use codepilotx_extension_api::ToolCall;
use codepilotx_extension_api::ToolCallOutcome;
use codepilotx_extension_api::ToolCallSource;
use codepilotx_extension_api::ToolExecutor;
use codepilotx_extension_api::ToolFinishInput;
use codepilotx_extension_api::ToolPayload;
use codepilotx_extension_api::TurnErrorInput;
use codepilotx_extension_api::TurnStartInput;
use codepilotx_extension_api::TurnStopInput;
use codepilotx_goal_extension::GoalObjectiveUpdate;
use codepilotx_goal_extension::GoalRuntimeHandle;
use codepilotx_goal_extension::GoalService;
use codepilotx_goal_extension::GoalSetRequest;
use codepilotx_goal_extension::GoalTokenBudgetUpdate;
use codepilotx_goal_extension::install_with_backend;
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(codepilotx_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(codepilotx_state::ThreadGoalStatus::Active, goal.status);
...
async fn installed_tools(
    runtime: Arc<codepilotx_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: codepilotx_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = codepilotx_extension_api::ToolName::plain(tool_name);
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: codepilotx_extension_api::ToolName::plain(tool_name),
...
async fn test_runtime() -> anyhow::Result<Arc<codepilotx_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    codepilotx_state::StateRuntime::init(tempdir.keep(), "test-provider".to_string()).await
}
...
async fn seed_thread_metadata(
    ...
    let builder = codepilotx_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .codepilotx_home()
...
fn protocol_status(status: codepilotx_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
