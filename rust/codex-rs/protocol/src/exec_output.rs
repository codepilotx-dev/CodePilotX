...
// Cyrillic garbage () in VS Code. However, CP866 uppercase tokens are perfectly valid output
// (e.g., ` test`) so we cannot flip every 0x80-0x9F byte to Windows-1252 either. The compromise
...
fn detect_encoding(bytes: &[u8]) -> &'static Encoding {
    ...
    // - CP866 mapping 0x93/0x94/0x96 to Cyrillic letters, so the same bytes show up as  when
