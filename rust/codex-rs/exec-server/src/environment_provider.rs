...
use crate::environment::codepilotx_EXEC_SERVER_URL_ENV_VAR;
...
/// Default provider backed by `codepilotx_EXEC_SERVER_URL`.
#[derive(Clone, Debug)]
...
impl DefaultEnvironmentProvider {
    /// Builds a provider from an already-read raw `codepilotx_EXEC_SERVER_URL` value.
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self { exec_server_url }
    }

    /// Builds a provider by reading `codepilotx_EXEC_SERVER_URL`.
    pub fn from_env() -> Self {
        Self::new(std::env::var(codepilotx_EXEC_SERVER_URL_ENV_VAR).ok())
    }
