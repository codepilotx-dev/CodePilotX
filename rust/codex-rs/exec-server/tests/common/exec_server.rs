...
pub(crate) struct TestCodexHelperPaths {
    pub(crate) codepilotx_exe: PathBuf,
    pub(crate) codepilotx_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_codepilotx_helper_paths() -> anyhow::Result<TestCodexHelperPaths> {
    let (helper_binary, codepilotx_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestCodexHelperPaths {
        codepilotx_exe: helper_binary,
...
{
    let helper_paths = test_codepilotx_helper_paths()?;
    let codepilotx_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.codepilotx_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("codepilotx_HOME", codepilotx_home.path());
    child.envs(env);
