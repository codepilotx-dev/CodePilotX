...
async fn thread_start_creates_thread_and_emits_started() -> Result<()> {
    ...
    create_config_toml_without_approval_policy(codepilotx_home.path(), &server.uri())?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let codepilotx_home = TempDir::new()?;
    create_config_toml_without_approval_policy(codepilotx_home.path(), &server.uri())?;
    let global_agents_path = codepilotx_home.path().join("AGENTS.md");
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let codepilotx_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(codepilotx_home.path(), &server.uri())?;
...
"#,
    ...
    let config_after = std::fs::read_to_string(codepilotx_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
