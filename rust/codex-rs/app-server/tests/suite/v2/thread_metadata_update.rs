...
use codepilotx_app_server_protocol::GitInfo;
...
use codepilotx_protocol::protocol::GitInfo as RolloutGitInfo;
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_stored_thread() -> Result<()> {
    ...
    let codepilotx_home = TempDir::new()?;
    create_config_toml(codepilotx_home.path(), &server.uri())?;
    let _state_db = init_state_db(codepilotx_home.path()).await?;
...
async fn thread_metadata_update_repairs_loaded_thread_without_resetting_summary() -> Result<()> {
    ...
    let codepilotx_home = TempDir::new()?;
    create_config_toml(codepilotx_home.path(), &server.uri())?;
    let state_db = init_state_db(codepilotx_home.path()).await?;
    ...
    let rollout_path = rollout_path(codepilotx_home.path(), "2025-01-06T08-30-00", &thread_id);
    reconcile_rollout(
        Some(&state_db),
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_archived_thread() -> Result<()> {
    ...
    let archived_source = rollout_path(codepilotx_home.path(), "2025-01-06T08-30-00", &thread_id);
