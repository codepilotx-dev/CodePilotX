...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    let codepilotx_home = TempDir::new()?;
    create_config_toml_with_approval_policy(codepilotx_home.path(), &server.uri(), "untrusted")?;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": codepilotx_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
fn create_config_toml(codepilotx_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    create_config_toml_with_approval_policy(codepilotx_home, server_uri, "never")
}
