...
async fn consume_rate_limit_reset_credit_requires_chatgpt_auth() -> Result<()> {
    ...
    let mut mcp = initialized_app_server(codepilotx_home.path()).await?;
...
async fn consume_account_rate_limit_reset_credit_maps_backend_outcomes() -> Result<()> {
    let (codepilotx_home, server) = chatgpt_test_context().await?;
    let cases = [
        (
            "request-reset",
            "reset",
...
async fn consume_account_rate_limit_reset_credit_rejects_empty_idempotency_key() -> Result<()> {
    let (codepilotx_home, _server) = chatgpt_test_context().await?;
    let mut mcp = initialized_app_server(codepilotx_home.path()).await?;
...
async fn chatgpt_test_context() -> Result<(TempDir, MockServer)> {
    ...
    write_chatgpt_base_url(codepilotx_home.path(), &server.uri())?;
    Ok((codepilotx_home, server))
}

async fn initialized_app_server(codepilotx_home: &Path) -> Result<TestAppServer> {
    let mut mcp = TestAppServer::new_with_env(codepilotx_home, &[("OPENAI_API_KEY", None)]).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    Ok(mcp)
}
...
fn write_chatgpt_base_url(codepilotx_home: &Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
