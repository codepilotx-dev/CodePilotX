    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                codepilotx_home.path().join("marketplace.json"),
            )?),
...
"#,
    ...
    write_installed_plugin(&codepilotx_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(codepilotx_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let codepilotx_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&codepilotx_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "codex-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(codepilotx_home.path().join(".tmp"))?;
    std::fs::write(
        codepilotx_home
            .path()
...
"#,
    ...
    write_installed_plugin(&codepilotx_home, "codex-curated", "demo-plugin")?;
...
async fn plugin_read_hides_apps_for_api_key_auth() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_env(
        codepilotx_home.path(),
        &[
            ("codepilotx_ACCESS_TOKEN", None),
            ("codepilotx_API_KEY", None),
            ("OPENAI_API_KEY", None),
        ],
    )
    .await?;
...
fn write_plugins_enabled_config(codepilotx_home: &TempDir) -> Result<()> {
    std::fs::write(
        codepilotx_home.path().join("config.toml"),
        r#"[features]
plugins = true
