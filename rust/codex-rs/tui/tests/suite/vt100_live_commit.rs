...
fn live_001_commit_on_overflow() {
    ...
    let mut term =
        codepilotx_tui::Terminal::with_options(backend).expect("terminal should be constructed");
    let area = Rect::new(
    ...
    let mut rb = codepilotx_tui::RowBuilder::new(/*target_width*/ 20);
    ...
    codepilotx_tui::insert_history_lines(&mut term, lines)
