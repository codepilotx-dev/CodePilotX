...
#[cfg(target_os = "windows")]
pub(crate) use codepilotx_windows_sandbox::sandbox_setup_is_complete;

#[cfg(not(target_os = "windows"))]
pub(crate) fn sandbox_setup_is_complete(_codepilotx_home: &Path) -> bool {
    false
}
...
pub(crate) fn run_elevated_setup(
    ...
    let permissions = codepilotx_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
        permission_profile,
        workspace_roots,
    )?;
...
pub(crate) fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    ...
    Some((
        ...
        codepilotx_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message),
    ))
}
