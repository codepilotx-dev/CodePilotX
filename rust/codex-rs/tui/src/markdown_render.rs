...
const TABLE_HEADER_SEPARATOR_CHAR: char = 'â”?;
const TABLE_BODY_SEPARATOR_CHAR: char = 'â”€';
...
// TableCell mutators inlined â€?called per-span during table event parsing.
impl TableCell {
    #[inline]
    ...
    I: Iterator<Item = (Event<'a>, Range<usize>)>,
{
    ...
    fn handle_event(&mut self, event: Event<'a>, range: Range<usize>) {
        ...
        match event {
            ...
            Event::Rule => {
                ...
                self.push_line(Line::from("â€”â€”â€?));
    ...
    fn text(&mut self, text: CowStr<'a>) {
        ...
        // Append verbatim â€?pulldown-cmark text events already contain the
    ...
    // Width-measurement helpers inlined â€?called per-cell during table column
...
mod tests {
    ...
    fn crlf_code_block_no_extra_blank_lines() {
        ...
        // The buffer must concatenate them verbatim â€?no inserted separators.
        let markdown = "```rust\r\nfn main() {}\r\n    line2\r\n```\r\n";
        let rendered = render_markdown_text(markdown);
        let lines = lines_to_strings(&rendered);
    ...
    fn column_classification_narrative_by_word_count() {
        ...
        // Col 1: prose (â‰? words per cell) â†?Narrative
    ...
    fn preferred_floor_compact_uses_body_token() {
        ...
        // Body token exceeds 16 cap â†?capped at 16, then max with header
        let m2 = TableColumnMetrics {
            max_width: 30,
            header_token_width: 5,
            body_token_width: 20,
    ...
    fn spillover_detects_trailing_html_label() {
        // "HTML block:" with no next_row â†?trailing HTML label spillover
        let row = make_body_row(
            vec![make_cell("HTML block:"), make_cell(""), make_cell("")],
            /*has_table_pipe_syntax*/ false,
        );
    ...
    fn spillover_keeps_normal_multi_cell_row() {
        // 3 cells all non-empty â†?not spillover
        let row = make_body_row(
    ...
    fn spillover_keeps_label_when_next_is_not_html() {
        // cell 0 = "Status:" and next_row cell 0 = "ok" â†?not spillover (not HTML)
        let row = make_body_row(
