...
//! machine and treats the key stream as normal typing. When toggling from enabled 鈫?disabled, the
...
impl ChatComposer {
    ...
    pub(crate) fn on_history_entry_response(
            ...
            .on_entry_response(log_id, offset, entry, &self.app_event_tx)
        {
            HistoryEntryResponse::Found(entry) => {
                // Persistent 鈫?鈫?history is text-only (backwards-compatible and avoids persisting
                // attachments), but local in-session 鈫?鈫?history can rehydrate elements and image paths.
    ...
    pub fn handle_paste_image_path(&mut self, pasted: String) -> bool {
        ...
        // normalize_pasted_path already handles Windows 鈫?WSL path conversion,
    ...
    ///
    /// When transitioning from enabled 鈫?disabled, we "defuse" any in-flight burst state so it
    ...
    ///
    /// When `record_history` is true, the final submission is stored for 鈫?鈫?recall.
            ...
            for plugin in plugins {
                ...
                let description = plugin.description.clone().or_else(|| {
                    Some(if capability_labels.is_empty() {
                        ...
                        format!("Plugin 路 {}", capability_labels.join(" 路 "))
                    })
                });
...
mod tests {
        ...
        snapshot_composer_state(
            ...
            |composer| {
                ...
                composer.set_status_line(Some(Line::from(
                    "gpt-5.4 high fast 路 ~/code/codex-1 路 Context 0% used",
                )));
    ...
    fn test_current_at_token_basic_cases() {
        let test_cases = vec![
            ...
            // Unicode examples
            ("@陌stanbul", 3, Some("陌stanbul".to_string()), "Turkish text"),
            (
                "@test袡笑校.rs",
                8,
                Some("test袡笑校.rs".to_string()),
                "Mixed ASCII and Cyrillic",
            ),
            ("@璇?, 2, Some("璇?.to_string()), "Chinese character"),
            ("@馃憤", 2, Some("馃憤".to_string()), "Emoji token"),
    ...
    fn ascii_prefix_survives_non_ascii_followup() {
        ...
        let _ = composer.handle_key_event(KeyEvent::new(KeyCode::Char('銇?), KeyModifiers::NONE));

        let (result, _) =
            composer.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
        match result {
            InputResult::Submitted { text, .. } => assert_eq!(text, "1銇?),
            _ => panic!("expected Submitted"),
        }
    }
    ...
    fn non_ascii_char_inserts_immediately_without_burst_state() {
        ...
        assert_eq!(composer.draft.textarea.text(), "銇?);
        assert!(!composer.is_in_paste_burst());
    }
    ...
    fn non_ascii_burst_buffers_enter_and_flushes_multiline() {
        ...
        let _ = composer.handle_key_event(KeyEvent::new(KeyCode::Char('浣?), KeyModifiers::NONE));
        let _ = composer.handle_key_event(KeyEvent::new(KeyCode::Char('濂?), KeyModifiers::NONE));
        ...
        assert!(composer.draft.textarea.text().is_empty());
        let _ = flush_after_paste_burst(&mut composer);
        assert_eq!(composer.draft.textarea.text(), "浣犲ソ\nhi");
    }
    ...
    fn non_ascii_burst_preserves_ideographic_space_and_ascii() {
        ...
        assert!(composer.draft.textarea.text().is_empty());
        let _ = flush_after_paste_burst(&mut composer);
        assert_eq!(composer.draft.textarea.text(), "浣犮��濂絓nhi");
    }
    ...
    fn non_ascii_burst_buffers_large_multiline_mixed_ascii_and_unicode() {
        ...
        const LARGE_MIXED_PAYLOAD: &str = "澶╁湴鐜勯粍 瀹囧畽娲崚\n\
鏃ユ湀鐩堟槂 杈板鍒楀紶\n\
瀵掓潵鏆戝線 绉嬫敹鍐棌\n\
\n\
浣犲ソ涓栫晫 缂栫爜娴嬭瘯\n\
...
绋嬪簭鍛?涓?Unicode 鍚岃";
    ...
    fn slash_popup_model_first_for_mo_ui() {
        ...
        // Type "/mo" humanlike so paste-burst doesn鈥檛 interfere.
    ...
    fn slash_popup_resume_for_res_ui() {
        ...
        // Type "/res" humanlike so paste-burst doesn鈥檛 interfere.
    ...
    fn backspace_with_multibyte_text_before_placeholder_does_not_panic() {
        ...
        composer.draft.textarea.insert_str("鏃ユ湰瑾?);
    ...
    fn slash_popup_activated_for_bare_slash_and_valid_prefixes() {
        ...
        // Case 4: invalid prefix "/zzz" 鈥?still allowed to open popup if it
