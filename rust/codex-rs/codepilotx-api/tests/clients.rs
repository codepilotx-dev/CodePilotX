...
use codepilotx_api::ResponsesClient;
use codepilotx_api::ResponsesOptions;
use codepilotx_client::HttpTransport;
use codepilotx_client::Request;
use codepilotx_client::RequestBody;
use codepilotx_client::Response;
use codepilotx_client::StreamResponse;
use codepilotx_client::TransportError;
...
fn provider(name: &str) -> Provider {
    Provider {
        ...
        query_params: None,
        headers: HeaderMap::new(),
        retry: codepilotx_api::RetryConfig {
            max_attempts: 1,
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, codepilotx_client::RequestCompression::None);
    Ok(())
}
