...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=codepilotx_BWRAP_SOURCE_DIR");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_ALLOW_CROSS");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_PATH");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_SYSROOT_DIR");
    println!("cargo:rerun-if-env-changed=codepilotx_SKIP_BWRAP_BUILD");
    ...
    if target_os != "linux" || env::var_os("codepilotx_SKIP_BWRAP_BUILD").is_some() {
        return;
    }
...
///
/// Priority:
/// 1. `codepilotx_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
...
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("codepilotx_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "codepilotx_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set codepilotx_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under codex-rs/vendor.",
        vendor_dir.display()
    ))
}
