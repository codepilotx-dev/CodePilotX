...
use crate::codepilotx_apps::write_cached_codepilotx_apps_tools;
...
use codepilotx_protocol::protocol::McpAuthStatus;
...
fn create_codepilotx_apps_tools_cache_context(
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    let mut tool = create_test_tool(codepilotx_APPS_MCP_SERVER_NAME, "upload").tool;
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        codepilotx_rmcp_client::Elicitation::Mcp(
...
#[test]
fn codepilotx_apps_tools_cache_is_overwritten_by_last_write() {
    let codepilotx_home = tempdir().expect("tempdir");
    let cache_context = create_codepilotx_apps_tools_cache_context(
        codepilotx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools_gateway_1 = vec![create_test_tool(codepilotx_APPS_MCP_SERVER_NAME, "one")];
...
#[test]
fn codepilotx_apps_tools_cache_is_scoped_per_user() {
    let codepilotx_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_codepilotx_apps_tools_cache_context(
        codepilotx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_codepilotx_apps_tools_cache_context(
        codepilotx_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(codepilotx_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(codepilotx_APPS_MCP_SERVER_NAME, "two")];

    write_cached_codepilotx_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_codepilotx_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 =
        read_cached_codepilotx_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_codepilotx_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn codepilotx_apps_tools_cache_preserves_formerly_disallowed_connectors() {
    let codepilotx_home = tempdir().expect("tempdir");
    let cache_context = create_codepilotx_apps_tools_cache_context(
        codepilotx_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn codepilotx_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let codepilotx_home = tempdir().expect("tempdir");
    let cache_context = create_codepilotx_apps_tools_cache_context(
        codepilotx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": codepilotx_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(codepilotx_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_codepilotx_apps_tools(&cache_context).is_none());
}

#[test]
fn codepilotx_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let codepilotx_home = tempdir().expect("tempdir");
    let cache_context = create_codepilotx_apps_tools_cache_context(
        codepilotx_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_codepilotx_apps_tools_loads_from_disk_cache() {
    let codepilotx_home = tempdir().expect("tempdir");
    let cache_context = create_codepilotx_apps_tools_cache_context(
        codepilotx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_codepilotx_apps_tools_if_needed(
        codepilotx_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
        &server_info,
        &cached_tools,
    ...
    let startup_tools = load_startup_cached_codepilotx_apps_tools_snapshot(
        codepilotx_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
    )
    .expect("expected startup snapshot to load from cache");
    let cached_server_info = load_startup_cached_codepilotx_apps_server_info(
        codepilotx_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
    );
...
#[test]
fn startup_cached_codepilotx_apps_tools_loads_without_server_info_cache() {
    let codepilotx_home = tempdir().expect("tempdir");
    let cache_context = create_codepilotx_apps_tools_cache_context(
        codepilotx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": codepilotx_APPS_TOOLS_CACHE_SCHEMA_VERSION,
        "tools": [create_test_tool(codepilotx_APPS_MCP_SERVER_NAME, "calendar_search")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");
...
#[test]
fn codepilotx_apps_server_info_cache_survives_legacy_tools_cache_write() {
    let codepilotx_home = tempdir().expect("tempdir");
    let cache_context = create_codepilotx_apps_tools_cache_context(
        codepilotx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    assert_eq!(
        load_startup_cached_codepilotx_apps_server_info(
            codepilotx_APPS_MCP_SERVER_NAME,
            Some(&cache_context),
        ),
        Some(server_info)
    ...
    assert!(
        load_startup_cached_codepilotx_apps_tools_snapshot(
            codepilotx_APPS_MCP_SERVER_NAME,
            Some(&cache_context),
        )
        .is_none()
...
async fn list_all_tools_uses_cached_tool_info_snapshot_while_client_is_pending() {
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__codepilotx_apps", "calendar_create_event")
        })
        .expect("tool from startup cache");
    assert_eq!(tool.server_name, codepilotx_APPS_MCP_SERVER_NAME);
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    assert_eq!(
        server_infos.get(codepilotx_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
async fn list_all_tools_uses_cached_tool_info_snapshot_when_client_startup_fails() {
    ...
    assert_eq!(
        manager
            ...
            .await
            .get(codepilotx_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
fn server_metadata_preserves_tool_approval_policy() {
    let mut config = crate::codepilotx_apps_mcp_server_config(
        "https://docs.example",
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    let manager = McpConnectionManager::new(
        ...
        /*host_owned_codepilotx_apps_enabled*/ false,
        /*prefix_mcp_tool_names*/ true,
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = codepilotx_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
