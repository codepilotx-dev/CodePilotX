...
use crate::codepilotx_apps::CodexAppsToolsCacheKey;
...
use codepilotx_protocol::protocol::McpStartupCompleteEvent;
use codepilotx_protocol::protocol::McpStartupFailure;
use codepilotx_protocol::protocol::McpStartupStatus;
use codepilotx_protocol::protocol::McpStartupUpdateEvent;
...
pub struct McpConnectionManager {
    ...
    host_owned_codepilotx_apps_enabled: bool,
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        codepilotx_apps_tools_cache_key: CodexAppsToolsCacheKey,
        host_owned_codepilotx_apps_enabled: bool,
        prefix_mcp_tool_names: bool,
        ...
        let codepilotx_apps_auth_provider = auth
            .filter(|auth| auth.uses_codepilotx_backend())
            .map(codepilotx_model_provider::auth_provider_from_auth);
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            let runtime_auth_provider =
                if server_name == codepilotx_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
                    codepilotx_apps_auth_provider.clone()
                } else {
                    None
                };
            let async_managed_client = AsyncManagedClient::new(
                ...
                codepilotx_apps_tools_cache_context,
    ...
    pub fn tool_approval_mode(
        ...
        server_name: &str,
        tool_name: &str,
    ) -> codepilotx_config::AppToolApproval {
        self.server_metadata
            .get(server_name)
    ...
    pub fn is_host_owned_codepilotx_apps_server(&self, server_name: &str) -> bool {
        self.host_owned_codepilotx_apps_enabled && server_name == codepilotx_APPS_MCP_SERVER_NAME
    }
    ...
    pub async fn hard_refresh_codepilotx_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let managed_client = self
            .clients
            .get(codepilotx_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{codepilotx_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{codepilotx_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_codepilotx_apps_tools_if_needed(
            codepilotx_APPS_MCP_SERVER_NAME,
            managed_client.codepilotx_apps_tools_cache_context.as_ref(),
