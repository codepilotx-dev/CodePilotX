...
impl McpRuntimeContext {
    ...
    pub(crate) fn resolve_server_environment(
        &self,
        server_name: &str,
        config: &codepilotx_config::McpServerConfig,
        ...
        if config.is_local_environment() {
            return match config.transport {
                codepilotx_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                codepilotx_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }
...
fn ensure_remote_stdio_cwd(
    ...
    let codepilotx_config::McpServerTransportConfig::Stdio { cwd, .. } = &config.transport else {
        return Ok(());
    };
...
mod tests {
    ...
    use codepilotx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
