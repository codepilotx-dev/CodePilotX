...
use crate::codepilotx_apps::codepilotx_apps_tools_cache_key;
use crate::connection_manager::McpConnectionManager;
use crate::runtime::McpRuntimeContext;
use crate::server::EffectiveMcpServer;

pub const codepilotx_APPS_MCP_SERVER_NAME: &str = "codepilotx_apps";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const codepilotx_CONNECTORS_TOKEN_ENV_VAR: &str = "codepilotx_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `codepilotx_core::config::Config`.
///
...
pub fn host_owned_codepilotx_apps_enabled(config: &McpConfig, auth: Option<&CodexAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(CodexAuth::uses_codepilotx_backend)
}
...
pub fn effective_mcp_servers_from_configured(
    ...
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_codepilotx_apps_enabled(config, auth) {
        servers.remove(codepilotx_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
pub async fn read_mcp_resource(
    ...
    let host_owned_codepilotx_apps_enabled = host_owned_codepilotx_apps_enabled(config, auth);
...
fn normalize_codepilotx_apps_base_url(base_url: &str) -> String {
...
fn codepilotx_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_codepilotx_apps_base_url(base_url);
...
pub fn codepilotx_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        codepilotx_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    let base_url = normalize_codepilotx_apps_base_url(chatgpt_base_url);
...
fn mcp_server_config_for_url(url: String, apps_mcp_product_sku: Option<&str>) -> McpServerConfig {
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: codepilotx_apps_mcp_bearer_token_env_var(),
            http_headers,
