...
fn test_mcp_config(codepilotx_home: PathBuf) -> McpConfig {
    McpConfig {
...
#[test]
fn codepilotx_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
        codepilotx_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/wham/apps"
    );
    assert_eq!(
        codepilotx_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/wham/apps"
    );
    assert_eq!(
        codepilotx_apps_mcp_url_for_base_url("http://localhost:8080/api/codex"),
        "http://localhost:8080/api/codex/apps"
    );
    assert_eq!(
        codepilotx_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/codex/apps"
    );
}
...
#[test]
fn codepilotx_apps_server_config_forwards_configured_product_sku_header() {
    let config = codepilotx_apps_mcp_server_config("https://chatgpt.com", Some("tpp"));
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(codepilotx_home.path().to_path_buf());
    ...
    let docs = docs
        .configured_config()
        .expect("configured server should retain transport");
    let codepilotx_apps = codepilotx_apps
        .configured_config()
    ...
    match &codepilotx_apps.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => {
            assert_eq!(url, "https://chatgpt.com/backend-api/wham/apps");
        }
        other => panic!("expected streamable http transport, got {other:?}"),
