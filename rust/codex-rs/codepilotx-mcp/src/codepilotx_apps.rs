...
use codepilotx_utils_plugins::mcp_connector::sanitize_name;
...
pub fn codepilotx_apps_tools_cache_key(auth: Option<&CodexAuth>) -> CodexAppsToolsCacheKey {
...
impl CodexAppsToolsCacheContext {
    pub(crate) fn tools_cache_path(&self) -> PathBuf {
        self.cache_path_in(codepilotx_APPS_TOOLS_CACHE_DIR)
    }

    pub(crate) fn server_info_cache_path(&self) -> PathBuf {
        self.cache_path_in(codepilotx_APPS_SERVER_INFO_CACHE_DIR)
    }
...
pub(crate) fn normalize_codepilotx_apps_tool_title(
    server_name: &str,
...
pub(crate) fn normalize_codepilotx_apps_callable_name(
    server_name: &str,
    tool_name: &str,
...
pub(crate) fn normalize_codepilotx_apps_callable_namespace(
    server_name: &str,
    connector_name: Option<&str>,
) -> String {
    if server_name == codepilotx_APPS_MCP_SERVER_NAME
...
pub(crate) fn write_cached_codepilotx_apps_tools_if_needed(
    server_name: &str,
    ...
    if let Some(cache_context) = cache_context {
        ...
        if let Err(err) = write_cached_codepilotx_apps_server_info(cache_context, server_info) {
...
pub(crate) fn load_startup_cached_codepilotx_apps_tools_snapshot(
    server_name: &str,
    cache_context: Option<&CodexAppsToolsCacheContext>,
) -> Option<Vec<ToolInfo>> {
    if server_name != codepilotx_APPS_MCP_SERVER_NAME {
...
pub(crate) fn load_startup_cached_codepilotx_apps_server_info(
    ...
    load_cached_codepilotx_apps_server_info(cache_context?)
}

#[cfg(test)]
pub(crate) fn read_cached_codepilotx_apps_tools(
...
pub(crate) fn load_cached_codepilotx_apps_tools(
    cache_context: &CodexAppsToolsCacheContext,
    ...
    if cache.schema_version != codepilotx_APPS_TOOLS_CACHE_SCHEMA_VERSION {
        return CachedCodexAppsToolsLoad::Invalid;
    }
    CachedCodexAppsToolsLoad::Hit(cache.tools)
}

pub(crate) fn write_cached_codepilotx_apps_tools(
    cache_context: &CodexAppsToolsCacheContext,
    tools: &[ToolInfo],
) {
    let cache_path = cache_context.tools_cache_path();
...
pub(crate) fn load_cached_codepilotx_apps_server_info(
    cache_context: &CodexAppsToolsCacheContext,
) -> Option<McpServerInfo> {
    ...
    (cache.schema_version == codepilotx_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION)
        .then_some(cache.server_info)
}

fn write_cached_codepilotx_apps_server_info(
    cache_context: &CodexAppsToolsCacheContext,
    server_info: &McpServerInfo,
) -> anyhow::Result<()> {
    ...
    let bytes = serde_json::to_vec_pretty(&CodexAppsServerInfoDiskCache {
        schema_version: codepilotx_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION,
...
const codepilotx_APPS_TOOLS_CACHE_DIR: &str = "cache/codepilotx_apps_tools";
pub(crate) const codepilotx_APPS_TOOLS_CACHE_SCHEMA_VERSION: u8 = 4;

const codepilotx_APPS_SERVER_INFO_CACHE_DIR: &str = "cache/codepilotx_apps_server_info";
const codepilotx_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION: u8 = 1;
