...
use crate::codepilotx_apps::CachedCodexAppsToolsLoad;
use crate::codepilotx_apps::CodexAppsToolsCacheContext;
use crate::codepilotx_apps::load_cached_codepilotx_apps_tools;
...
use crate::codepilotx_apps::normalize_codepilotx_apps_callable_name;
use crate::codepilotx_apps::normalize_codepilotx_apps_callable_namespace;
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) codepilotx_apps_tools_cache_context: Option<CodexAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.codepilotx_apps_tools_cache_context.as_ref()
            && let CachedCodexAppsToolsLoad::Hit(tools) =
                load_cached_codepilotx_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.codepilotx_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        codepilotx_apps_tools_cache_context: Option<CodexAppsToolsCacheContext>,
        ...
        let tool_filter = server
            ...
            .unwrap_or_default();
        let cached_tool_info_snapshot = load_startup_cached_codepilotx_apps_tools_snapshot(
            &server_name,
            codepilotx_apps_tools_cache_context.as_ref(),
        );
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            let callable_name = normalize_codepilotx_apps_callable_name(
                server_name,
                &tool_def.name,
...
mod tests {
    ...
    #[test]
    fn codepilotx_apps_connector_metadata_is_preserved() {
        let mut tool = tool_with_connector_meta();
