...
//! detailed explanation of what "hermetic" means here lives in `codepilotx_client::custom_ca`; these
...
const codepilotx_CA_CERT_ENV: &str = "codepilotx_CA_CERTIFICATE";
const PROBE_PROXY_ENV: &str = "codepilotx_CUSTOM_CA_PROBE_PROXY";
const PROBE_TLS13_ENV: &str = "codepilotx_CUSTOM_CA_PROBE_TLS13";
const PROBE_URL_ENV: &str = "codepilotx_CUSTOM_CA_PROBE_URL";
...
fn probe_command() -> Command {
    ...
    cmd.env_remove(codepilotx_CA_CERT_ENV);
...
fn spawn_tls13_test_server() -> Tls13TestServer {
    codepilotx_utils_rustls_provider::ensure_rustls_crypto_provider();
...
#[test]
fn uses_codepilotx_ca_cert_env() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);

    let output = run_probe(&[(codepilotx_CA_CERT_ENV, cert_path.as_path())]);
...
#[test]
fn prefers_codepilotx_ca_cert_over_ssl_cert_file() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("codepilotx_CA_CERTIFICATE"));
