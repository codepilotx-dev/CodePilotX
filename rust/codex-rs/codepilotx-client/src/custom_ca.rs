...
//!
//! - read CA material from `codepilotx_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
pub const codepilotx_CA_CERT_ENV: &str = "codepilotx_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set codepilotx_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// CA handling before finally constructing the client. `codepilotx_CA_CERTIFICATE` takes precedence
...
/// `codepilotx_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
fn build_reqwest_client_with_env(
    ...
    info!(
        codepilotx_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `codepilotx_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Codex-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(codepilotx_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: codepilotx_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
mod tests {
    ...
    #[test]
    fn ca_path_prefers_codepilotx_env() {
        let env = map_env(&[
            (codepilotx_CA_CERT_ENV, "/tmp/codex.pem"),
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(codepilotx_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);
