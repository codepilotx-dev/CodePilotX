...
pub async fn maybe_migrate_personality(
    ...
    let marker_path = codepilotx_home.join(PERSONALITY_MIGRATION_FILENAME);
    ...
    if !has_recorded_sessions(codepilotx_home, model_provider_id.as_str(), state_db).await? {
        create_marker(&marker_path).await?;
...
async fn has_threads(store: &LocalThreadStore, archived: bool) -> io::Result<bool> {
    store
        .list_threads(ListThreadsParams {
            ...
            sort_direction: codepilotx_thread_store::SortDirection::Desc,
