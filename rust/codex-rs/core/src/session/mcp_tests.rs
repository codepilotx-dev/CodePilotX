...
fn guardian_meta(tool_params: Option<Value>) -> Option<Meta> {
    let mut value = json!({
        "codepilotx_approval_kind": "mcp_tool_call",
        "codepilotx_request_type": "approval_request",
...
fn plugin_install_elicitation_telemetry_metadata_requires_install_tool_suggestion() {
    let event = EventMsg::ElicitationRequest(ElicitationRequestEvent {
        ...
        id: codepilotx_protocol::mcp::RequestId::String("request-1".to_string()),
        request: codepilotx_protocol::approvals::ElicitationRequest::Form {
            meta: Some(json!({
    ...
    let enable_event = EventMsg::ElicitationRequest(ElicitationRequestEvent {
        turn_id: Some("turn-1".to_string()),
        server_name: "codepilotx_apps".to_string(),
        id: codepilotx_protocol::mcp::RequestId::String("request-2".to_string()),
        request: codepilotx_protocol::approvals::ElicitationRequest::Form {
            meta: Some(json!({
                "codepilotx_approval_kind": "tool_suggestion",
                "suggest_type": "enable",
