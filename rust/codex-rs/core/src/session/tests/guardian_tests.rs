...
use codepilotx_protocol::models::AdditionalPermissionProfile as PermissionProfile;
...
use core_test_support::codepilotx_linux_sandbox_exe_or_skip;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                codepilotx_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    turn_context_raw.permission_profile = codepilotx_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config.codepilotx_linux_sandbox_exe = codepilotx_linux_sandbox_exe_or_skip!();
    config
        .features
        .enable(Feature::GuardianApproval)
        .expect("test setup should allow enabling guardian approvals");
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        codepilotx_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: codepilotx_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let codepilotx_home = tempdir().expect("create codex home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_codepilotx_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(codepilotx_thread_store::LocalThreadStore::new(
        codepilotx_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
