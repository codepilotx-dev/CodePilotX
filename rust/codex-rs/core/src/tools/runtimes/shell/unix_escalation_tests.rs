...
fn extract_shell_script_supports_wrapped_command_prefixes() {
    assert_eq!(
        extract_shell_script(&[
            "/usr/bin/env".into(),
            "codepilotx_EXECVE_WRAPPER=1".into(),
            "/bin/zsh".into(),
            "-lc".into(),
            "echo hello".into()
        ])
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(codepilotx_execpolicy::Policy::empty())),
        ...
        stopwatch: codepilotx_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = codepilotx_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = codepilotx_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    let expected_hook_command =
        codepilotx_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(codepilotx_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        codepilotx_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        codepilotx_shell_escalation::EscalationDecision::Escalate(
            codepilotx_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, codepilotx_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        codepilotx_shell_escalation::EscalationDecision::Deny {
