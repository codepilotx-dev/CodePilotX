...
use codepilotx_code_mode::CellId;
use codepilotx_code_mode::CodeModeNestedToolCall;
use codepilotx_code_mode::CodeModeSession;
use codepilotx_code_mode::CodeModeToolKind;
use codepilotx_code_mode::RuntimeResponse;
...
use codepilotx_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = codepilotx_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = codepilotx_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = codepilotx_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    ...
    pub(crate) async fn execute(
        &self,
        request: codepilotx_code_mode::ExecuteRequest,
    ) -> Result<codepilotx_code_mode::StartedCell, String> {
        self.session()?.execute(request).await
    }

    pub(crate) async fn wait(
        &self,
        request: codepilotx_code_mode::WaitRequest,
    ) -> Result<codepilotx_code_mode::WaitOutcome, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(&self, cell_id: &codepilotx_code_mode::CellId) {
