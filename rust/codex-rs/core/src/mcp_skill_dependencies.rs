...
use codepilotx_config::ConfigEditsBuilder;
use codepilotx_config::McpServerConfig;
use codepilotx_config::McpServerTransportConfig;
use codepilotx_config::load_global_mcp_servers;
use codepilotx_login::default_client::is_first_party_originator;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(codepilotx_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&codepilotx_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&codepilotx_home)
