...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(codepilotx_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = codepilotx_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let codepilotx_home = tmp.path();
    let link_a = codepilotx_home.join("a.toml");
    let link_b = codepilotx_home.join("b.toml");
    let config_path = codepilotx_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(codepilotx_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
#[test]
fn blocking_set_hide_gpt_5_1_codepilotx_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let codepilotx_home = tmp.path();
    std::fs::write(
        codepilotx_home.join(CONFIG_TOML_FILE),
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !codepilotx_home.join(CONFIG_TOML_FILE).exists(),
