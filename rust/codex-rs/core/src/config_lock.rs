...
pub(crate) struct ConfigLockReplayOptions {
    pub allow_codepilotx_version_mismatch: bool,
}
...
pub(crate) fn validate_config_lock_replay(
    ...
    if !options.allow_codepilotx_version_mismatch
        && expected_lock.codepilotx_version != actual_lock.codepilotx_version
    {
        return Err(config_lock_error(format!(
            "config lock Codex version mismatch: lock was generated by {}, current version is {}; set debug.config_lockfile.allow_codepilotx_version_mismatch=true to ignore this",
            expected_lock.codepilotx_version, actual_lock.codepilotx_version
        )));
    }
...
fn config_lock_for_comparison(
    ...
    if options.allow_codepilotx_version_mismatch {
        lockfile.codepilotx_version.clear();
    }
    lockfile
}
