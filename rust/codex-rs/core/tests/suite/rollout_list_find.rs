...
use chrono::Utc;
use codepilotx_core::RolloutRecorder;
use codepilotx_core::RolloutRecorderParams;
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(codepilotx_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(codepilotx_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    let runtime = StateRuntime::init(codepilotx_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = codepilotx_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_codepilotx_home_directory() {
    let repo = TempDir::new().unwrap();
    let codepilotx_home = repo.path().join(".codex");
