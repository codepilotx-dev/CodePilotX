...
const codepilotx_DMG_URL_ARM64: &str = "https://persistent.oaistatic.com/codex-app-prod/Codex.dmg";
const codepilotx_DMG_URL_X64: &str =
...
pub async fn run_mac_app_open_or_install(
    ...
    if let Some(app_path) = find_existing_codepilotx_app_path() {
        eprintln!(
            "Opening Codex Desktop at {app_path}...",
            app_path = app_path.display()
        );
        open_codepilotx_app(&app_path, &workspace).await?;
        return Ok(());
    }
    eprintln!("CodePilotX Desktop not found; downloading installer...");
    let download_url = download_url_override.unwrap_or_else(|| {
        let default_url = if is_apple_silicon_mac() {
            codepilotx_DMG_URL_ARM64
        } else {
            codepilotx_DMG_URL_X64
        };
        default_url.to_string()
    });
    let installed_app = download_and_install_codepilotx_to_user_applications(&download_url)
        .await
    ...
    open_codepilotx_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_codepilotx_app_path() -> Option<PathBuf> {
    candidate_codepilotx_app_paths()
        .into_iter()
        .find(|candidate| candidate.is_dir())
}

fn candidate_codepilotx_app_paths() -> Vec<PathBuf> {
...
async fn open_codepilotx_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = codepilotx_new_thread_url(workspace);
...
fn codepilotx_new_thread_url(workspace: &Path) -> String {
...
async fn download_and_install_codepilotx_to_user_applications(
    ...
    let result = async {
        let app_in_volume = find_codepilotx_app_in_mount(&mount_point)
            .context("failed to locate Codex.app in mounted dmg")?;
        install_codepilotx_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_codepilotx_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
...
fn find_codepilotx_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
...
#[cfg(test)]
mod tests {
    use super::codepilotx_new_thread_url;
