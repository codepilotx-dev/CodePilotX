...
async fn send_codepilotx_exec_originator() -> anyhow::Result<()> {
    ...
    responses::mount_sse_once_match(&server, header("Originator", "codepilotx_exec"), body).await;

    test.cmd_with_server(&server)
        .env_remove(codepilotx_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
