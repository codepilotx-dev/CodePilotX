# codepilotx-app-server-daemon

> `codepilotx-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`codepilotx-app-server-daemon` backs the machine-readable `codepilotx app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for CodePilotX instances launched over SSH, including fresh developer
...
```sh
codepilotx app-server daemon start
codepilotx app-server daemon restart
codepilotx app-server daemon enable-remote-control
codepilotx app-server daemon disable-remote-control
codepilotx app-server daemon stop
codepilotx app-server daemon version
codepilotx app-server daemon bootstrap --remote-control
```
...
curl -fsSL https://chatgpt.com/codex/install.sh | sh
	$HOME/.codepilotx/packages/standalone/current/codepilotx app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `CODEPILOTX_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes CodePilotX is installed through `install.sh` and always launches
the standalone managed binary under `CODEPILOTX_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `CODEPILOTX_HOME/packages/standalone/current/codepilotx` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `CODEPILOTX_HOME/packages/standalone/current/codepilotx` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `codepilotx remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `CODEPILOTX_HOME`, so a concurrent
...
The daemon stores its local state under `CODEPILOTX_HOME/app-server-daemon/`:
