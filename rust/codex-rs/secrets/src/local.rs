...
const codepilotx_AUTH_SECRETS_FILENAME: &str = "codepilotx_auth.age";
...
impl LocalSecretsBackend {
    pub fn new(codepilotx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn secrets_dir(&self) -> PathBuf {
        self.codepilotx_home.join("secrets")
    }

    fn secrets_path(&self) -> PathBuf {
        let filename = match self.namespace {
            LocalSecretsNamespace::ManagedSecrets => LOCAL_SECRETS_FILENAME,
            LocalSecretsNamespace::CodexAuth => codepilotx_AUTH_SECRETS_FILENAME,
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.codepilotx_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(codepilotx_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let codepilotx_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(codepilotx_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        let codepilotx_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let codepilotx_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        codepilotx_auth_backend.set(&scope, &name, "codex-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            codepilotx_auth_backend.get(&scope, &name)?,
