...
use codepilotx_tui::render_markdown_text;
...
fn rounded_enabled() -> bool {
    *ROUNDED.get_or_init(|| {
        std::env::var("codepilotx_TUI_ROUNDED")
            .ok()
            .map(|v| v == "1")
            .unwrap_or(true)
    })
...
fn draw_list(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    if app.refresh_inflight {
        draw_centered_spinner(frame, inner, &mut app.spinner_start, "Loading tasksâ€?);
    }
}
...
fn draw_footer(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    if app.refresh_inflight
        ...
        || app.apply_inflight
    {
        draw_inline_spinner(frame, top[1], &mut app.spinner_start, "Loadingâ€?);
    } else {
...
pub fn draw_apply_modal(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    if let Some(m) = &app.apply_modal {
        ...
        if app.apply_preflight_inflight {
            draw_centered_spinner(frame, rows[1], &mut app.spinner_start, "Checkingâ€?);
        } else if app.apply_inflight {
            draw_centered_spinner(frame, rows[1], &mut app.spinner_start, "Applyingâ€?);
        } else if m.result_message.is_none() {
            draw_centered_spinner(frame, rows[1], &mut app.spinner_start, "Loadingâ€?);
            ...
            if !matches!(m.result_level, Some(crate::app::ApplyResultLevel::Success)) {
                ...
                if !m.conflict_paths.is_empty() {
                    ...
                    for p in &m.conflict_paths {
                        body_lines
                            .push(Line::from(vec!["  â€?".into(), Span::raw(p.clone()).dim()]));
                    }
                }
...
fn conversation_header_line(
    ...
    let mut spans: Vec<Span> = vec!["â•?".dim()];
    match speaker {
        ConversationSpeaker::User => {
...
fn conversation_gutter_span(speaker: ConversationSpeaker) -> ratatui::text::Span<'static> {
    match speaker {
        ConversationSpeaker::User => "â”?".cyan().dim(),
        ConversationSpeaker::Assistant => "â”?".magenta().dim(),
    }
}
...
pub fn draw_best_of_modal(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    let hint = Paragraph::new(Line::from("Use â†?â†?to choose, 1-4 jump".cyan().dim()))
        .wrap(Wrap { trim: true });
