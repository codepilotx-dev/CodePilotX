...
mod tests {
    ...
    fn stable_schema_filter_removes_mock_thread_start_field() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("codepilotx_schema_{}", Uuid::now_v7()));
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        let bundle_json =
            fs::read_to_string(output_dir.join("codepilotx_app_server_protocol.schemas.json"))?;
        ...
        let flat_v2_bundle_json =
            fs::read_to_string(output_dir.join("codepilotx_app_server_protocol.v2.schemas.json"))?;
        ...
        let flat_v2_bundle =
            read_json_value(&output_dir.join("codepilotx_app_server_protocol.v2.schemas.json"))?;
