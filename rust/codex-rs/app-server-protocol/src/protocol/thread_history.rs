...
use codepilotx_protocol::protocol::AgentReasoningEvent;
use codepilotx_protocol::protocol::AgentReasoningRawContentEvent;
...
use codepilotx_protocol::protocol::ContextCompactedEvent;
...
use codepilotx_protocol::protocol::ImageGenerationBeginEvent;
...
use codepilotx_protocol::protocol::ViewImageToolCallEvent;
use codepilotx_protocol::protocol::WebSearchBeginEvent;
use codepilotx_protocol::protocol::WebSearchEndEvent;
...
#[cfg(test)]
use codepilotx_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use codepilotx_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    fn handle_response_item(&mut self, item: &codepilotx_protocol::models::ResponseItem) {
    ...
    fn handle_item_started(&mut self, payload: &ItemStartedEvent) {
        match &payload.item {
            codepilotx_protocol::items::TurnItem::Plan(plan) => {
            ...
            codepilotx_protocol::items::TurnItem::Sleep(_) => {
                self.upsert_item_in_turn_id(
                    &payload.turn_id,
                    ThreadItem::from(payload.item.clone()),
                );
            }
            codepilotx_protocol::items::TurnItem::UserMessage(_)
            | codepilotx_protocol::items::TurnItem::HookPrompt(_)
            | codepilotx_protocol::items::TurnItem::AgentMessage(_)
            | codepilotx_protocol::items::TurnItem::Reasoning(_)
            | codepilotx_protocol::items::TurnItem::WebSearch(_)
            | codepilotx_protocol::items::TurnItem::ImageView(_)
            | codepilotx_protocol::items::TurnItem::ImageGeneration(_)
            | codepilotx_protocol::items::TurnItem::FileChange(_)
            | codepilotx_protocol::items::TurnItem::McpToolCall(_)
            | codepilotx_protocol::items::TurnItem::ContextCompaction(_) => {}
        }
    }
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &codepilotx_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &codepilotx_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &codepilotx_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &codepilotx_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &codepilotx_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &codepilotx_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &codepilotx_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &codepilotx_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &codepilotx_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &codepilotx_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &codepilotx_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &codepilotx_protocol::protocol::ExitedReviewModeEvent,
    ) {
        let review = payload
            .review_output
            .as_ref()
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        let changed_turn = if let Some(turn) = self.current_turn.as_mut() {
            ...
            turn.error = Some(V2TurnError {
                ...
                codepilotx_error_info: payload.codepilotx_error_info.clone().map(Into::into),
...
fn convert_dynamic_tool_content_items(
    items: &[codepilotx_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
...
mod tests {
    ...
    use codepilotx_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use codepilotx_protocol::items::SleepItem as CoreSleepItem;
    use codepilotx_protocol::items::TurnItem as CoreTurnItem;
    use codepilotx_protocol::items::UserMessageItem as CoreUserMessageItem;
    ...
    use codepilotx_protocol::models::MessagePhase as CoreMessagePhase;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![codepilotx_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                codepilotx_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
                    namespace: Some("codepilotx_app".into()),
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(codepilotx_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(codepilotx_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                codepilotx_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                codepilotx_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                codepilotx_error_info: Some(CodexErrorInfo::ResponseStreamDisconnected {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(codepilotx_protocol::models::ResponseItem::Message {
                id: Some("msg-1".into()),
                role: "user".into(),
