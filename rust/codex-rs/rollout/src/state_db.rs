...
pub use codepilotx_state::LogEntry;
...
pub type StateDbHandle = Arc<codepilotx_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    .await;
    codepilotx_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == codepilotx_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, codepilotx_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = codepilotx_state::state_db_path(config.sqlite_home());
    ...
    let runtime = match codepilotx_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    metrics: codepilotx_otel::MetricsClient,
    originator: &str,
) -> codepilotx_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == codepilotx_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<codepilotx_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&codepilotx_state::StateRuntime>,
    codepilotx_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.codepilotx_home() != codepilotx_home {
        warn!(
            "state db codepilotx_home mismatch: expected {}, got {}",
            ctx.codepilotx_home().display(),
            codepilotx_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => codepilotx_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => codepilotx_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => codepilotx_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
...
) -> Option<codepilotx_state::ThreadsPage> {
    let ctx = context?;
    if ctx.codepilotx_home() != codepilotx_home {
        warn!(
            "state db codepilotx_home mismatch: expected {}, got {}",
    ...
    let filters = codepilotx_state::ThreadFilterOptions {
        ...
        sort_direction: match sort_direction {
            SortDirection::Asc => codepilotx_state::SortDirection::Asc,
            SortDirection::Desc => codepilotx_state::SortDirection::Desc,
        },
        search_term,
    };
