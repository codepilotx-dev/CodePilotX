...
pub fn cap_sid_file(codepilotx_home: &Path) -> PathBuf {
    codepilotx_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(codepilotx_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(codepilotx_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(codepilotx_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(codepilotx_home);
    let mut caps = load_or_create_cap_sids(codepilotx_home)?;
...
pub fn writable_root_cap_sid_for_path(codepilotx_home: &Path, root: &Path) -> Result<String> {
    let path = cap_sid_file(codepilotx_home);
    let mut caps = load_or_create_cap_sids(codepilotx_home)?;
...
pub fn workspace_write_cap_sid_for_root(
    ...
    if canonical_path_key(root) == canonical_path_key(cwd) {
        workspace_cap_sid_for_cwd(codepilotx_home, cwd)
    } else {
        writable_root_cap_sid_for_path(codepilotx_home, root)
    }
}
