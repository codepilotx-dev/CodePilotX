...
impl ToolOutput for codepilotx_protocol::mcp::CallToolResult {
    fn log_preview(&self) -> String {
    ...
    match response {
        ResponseInputItem::Message { content, .. } => content_items_to_code_mode_result(
            &content
                .into_iter()
                .map(|item| match item {
                    codepilotx_protocol::models::ContentItem::InputText { text }
                    | codepilotx_protocol::models::ContentItem::OutputText { text } => {
                        FunctionCallOutputContentItem::InputText { text }
                    }
                    codepilotx_protocol::models::ContentItem::InputImage { image_url, detail } => {
